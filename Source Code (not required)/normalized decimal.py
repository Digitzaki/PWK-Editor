...
def main():
    ...
    # Function to update the alpha based on RGB values and alpha multiplier
    def update_alpha():
        global alpha
        alpha = 1 - (r + g + b) / 3
        alpha = 1 - alpha
    ...
    # Function to handle alpha slider
    def update_alpha_slider(val):
        global alpha_multiplier, a
        alpha_multiplier = 1 + (float(val) - 1) * 0.25 #placeholder 25% 
        a = int(val)
        alpha_hex_label.config(text=f"LR=KA? (Alpha) Hex: {format_hex_editor_value(a)}")
        update_alpha()
        update_color()
    ...
    # Canvas for displaying the color circle with more space for HEX
    canvas = tk.Canvas(root, width=350, height=350)
    canvas.grid(row=0, column=1, rowspan=6, padx=10, pady=10)

    # HEX color label under the circle
